...
pub const API_HOST: &str = "https://api.anilibria.tv/v3";
...
pub fn fetch_updates_list(page: u32) -> Result<(Pagination, Vec<Title>), reqwest::Error> {
    ...
    let resp = reqwest::blocking::get(format!(
        "{API_HOST}/v3/title/updates?filter=names,player,list,id&limit=9&page={page}"
    ))?;
...
fn process_server_response_body(response: reqwest::blocking::Response) -> (Pagination, Vec<Title>) {
    let mut jsonVal: Value = serde_json::from_str(response.text().unwrap().as_str())
        .expect("error while parsing response");
    let titles =
        serde_json::from_value(jsonVal["list"].take()).expect("error while parsing response");
    let pages: Pagination =
        serde_json::from_value(jsonVal["pagination"].take()).expect("error while parsing response");
    return (pages, titles);
}
